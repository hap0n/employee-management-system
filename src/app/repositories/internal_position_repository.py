...
class InternalPositionRepository(BaseRepository):
    ...
    @classmethod
    async def _save(cls, connection, internal_position: InternalPosition):
        records = await connection.fetchrow(
            f"INSERT INTO {cls.TABLE_NAME} (position_id, reports_to) VALUES ($1, $2) RETURNING *",
            internal_position.position_id,
            internal_position.reports_to,
        )
        return records

    @classmethod
    async def _update(cls, connection, internal_position: InternalPosition):
        records = await connection.fetchrow(
            f"UPDATE {cls.TABLE_NAME} SET position_id=$2, reports_to=$3 WHERE id=$1 RETURNING *",
            internal_position.id,
            internal_position.position_id,
            internal_position.reports_to,
        )
        return records
